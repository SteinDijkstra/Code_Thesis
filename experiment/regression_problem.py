...
def run_experiment(data, testdata, options):
    # Create folder with date+time+regression & 4 subfolders
    outputfolder = init_folder(data, testdata, options)

    # regression
    model_evidential, result_evidential, time_evidential = regression_evidential(
    ...
    model_evidential_new, result_evidential_new, time_evidential_new = (
        regression_evidential_new(data, testdata, options)
    )
    saveresult(
        options,
    ...
    # regression
    model_dropout, result_dropout, time_dropout = regression_mcdropout(
        data, testdata, options
    )
    saveresult(
    ...
    # regression
    model_ensemble, result_ensemble, time_ensemble = regression_ensemble(
        data, testdata, options
    )
    saveresult(
        options,
...
def regression_evidential(data, testdata, options):
    model_evidential = evidential.create_model_regression(options)
...
def regression_evidential_new(data, testdata, options):
    model_evidential_new = evidential_novel.create_model_regression(options)
...
def regression_mcdropout(data, testdata, options):
    model_mcdropout = mcdropout.create_model_regression(data, options)
...
def regression_ensemble(data, testdata, options):
    model_ensemble = ensemble.create_model_regression(options)
...
def init_folder(data, testdata, options):
    if options.outputoptions.save_output:
        outputdir = options.outputoptions.output_location
        experiment = f"Experiment_{len(os.listdir('./results'))+1}_regression"
